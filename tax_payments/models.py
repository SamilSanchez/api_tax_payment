...
class Tax(models.Model):
    ...
    description = models.TextField(
        ...
        help_text="Descripcion breve del servicio"
    )

    def __str__(self) -> str:
        return f"Refencia: {self.barcode} | Servicio: {self.service_type} | Estatus del pago: {self.payment_status}"
...
class TaxPayment(models.Model):
    ...
    amount = models.FloatField(verbose_name="Importe del pago", validators=[different_to_zero, validate_number_greater_than_zero])

    date = models.DateField(verbose_name="Fecha de pago")


    def clean(self) -> None:
        if self.pay_method == "cash" and self.card_number:
            raise ValidationError("El metodo de pago seleccionado fue en efectivo, debe borrar el campo de numero de tarjeta")
